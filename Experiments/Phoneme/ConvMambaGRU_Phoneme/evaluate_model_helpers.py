...
def runSingleDecodingStepMamba(neural_input, input_layer, trainer):
    x = torch.tensor(neural_input, device=trainer.device, dtype=torch.float32)
    n_time_steps = torch.tensor([x.shape[1]], device=trainer.device, dtype=torch.int32)
    day_idx = torch.tensor([input_layer], device=trainer.device, dtype=torch.long)
    ...
    with torch.autocast(
        ...
        x, n_time_steps = trainer.transform_data(x, n_time_steps, "val")

        with torch.no_grad():
            logits = trainer.model(x, day_idx)
