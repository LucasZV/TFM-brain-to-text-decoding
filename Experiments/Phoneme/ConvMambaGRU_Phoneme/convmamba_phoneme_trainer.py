...
from ConvMamba_model import MambaTextDecoder


def labels_minmax(labels_tensor):
    try:
        return f"{labels_tensor.min().item()} / {labels_tensor.max().item()}"
    except Exception:
        return "unavailable"


class BrainToPhonemeDecoderTrainer:
    """
    Trainer for brain-to-phoneme decoding with ConvMamba + CTC
    """
    ...
    def __init__(self, args):
        ...
        self.model = MambaTextDecoder(
            neural_dim=self.args["model"]["n_input_features"],
            n_classes=len(PHONE_VOCAB),
            n_days=len(self.args["dataset"]["sessions"]),
            hidden_dim=self.args["model"]["n_units"],
            n_layers=self.args["model"]["n_layers"],
            patch_size=self.args["model"]["patch_size"],
            patch_stride=self.args["model"]["patch_stride"],
            input_dropout=self.args["model"]["input_network"]["input_layer_dropout"],
            dropout=self.args["model"]["rnn_dropout"],
            conv_kernel_size=self.args["model"]["conv_kernel_size"],
            conv_dropout=self.args["model"]["conv_dropout"],
            mlp_ratio=self.args["model"]["mlp_ratio"],
        )

        self.logger.info("Initialized ConvMamba phoneme decoding model")
        ...
        for name, param in self.model.named_parameters():
            if not self.args["model"]["rnn_trainable"] and "mamba_blocks" in name:
    ...
    def create_optimizer(self):
        bias_params = [p for name, p in self.model.named_parameters() if name.endswith(".bias")]
    ...
    def load_model_checkpoint(self, load_path):
        ...
        self.best_val_PER = checkpoint["val_PER"] if "val_PER" in checkpoint else torch.inf
    ...
    def _log_batch_debug_info(self, i, batch, features, n_time_steps, adjusted_lens, phone_seq_lens, day_indicies):
        try:
            self.logger.info(f"Debug batch {i}")
            self.logger.info(f"day_indicies: {day_indicies.detach().cpu().tolist()}")
            self.logger.info(f"n_time_steps min/max: {n_time_steps.min().item()} / {n_time_steps.max().item()}")
            self.logger.info(f"adjusted_lens min/max: {adjusted_lens.min().item()} / {adjusted_lens.max().item()}")
            self.logger.info(f"phone_seq_lens min/max: {phone_seq_lens.min().item()} / {phone_seq_lens.max().item()}")
            self.logger.info(f"Any adjusted_lens <= 0? {bool((adjusted_lens <= 0).any().item())}")
            self.logger.info(f"Any phone_seq_lens <= 0? {bool((phone_seq_lens <= 0).any().item())}")
            self.logger.info(f"Any adjusted_lens < phone_seq_lens? {bool((adjusted_lens < phone_seq_lens).any().item())}")
            self.logger.info(f"features finite? {bool(torch.isfinite(features).all().item())}")
            self.logger.info(f"features abs max: {features.detach().abs().max().item():.6f}")
            self.logger.info(f"labels min/max: {labels_minmax(batch['seq_class_ids'])}")
        except Exception as e:
            self.logger.info(f"Could not log full debug info for batch {i}: {str(e)}")

    def _check_nonfinite_gradients(self, batch_idx):
        for name, p in self.model.named_parameters():
            if p.grad is not None and not torch.isfinite(p.grad).all():
                self.logger.info(f"Non-finite gradient detected at batch {batch_idx} in parameter: {name}")
                return True
        return False
    ...
    def train(self):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            if not torch.isfinite(features).all():
                ...
                self._log_batch_debug_info(i, batch, features, n_time_steps, n_time_steps.to(torch.int32), phone_seq_lens, day_indicies)
                continue
            ...
            with torch.autocast(device_type="cuda", enabled=self.args["use_amp"], dtype=torch.bfloat16):
                ...
                if not torch.isfinite(features).all():
                    ...
                    self._log_batch_debug_info(i, batch, features, n_time_steps, adjusted_lens, phone_seq_lens, day_indicies)
                    continue
                ...
                if (phone_seq_lens <= 0).any():
                    self.logger.info(f"Non-positive phone_seq_lens at batch {i}")
                    self._log_batch_debug_info(i, batch, features, n_time_steps, adjusted_lens, phone_seq_lens, day_indicies)
                    continue

                valid_mask = adjusted_lens >= phone_seq_lens
                ...
                if not valid_mask.all():
                    ...
                    self.logger.info(
                        ...
                        f"(keeping {n_valid}/{len(valid_mask)}) because adjusted_lens < phone_seq_lens"
                    )
                ...
                if not torch.isfinite(logits).all():
                    ...
                    self._log_batch_debug_info(i, batch, features, n_time_steps, adjusted_lens, phone_seq_lens, day_indicies)
                    self.logger.info(f"logits abs max: {logits.detach().abs().max().item():.6f}")
                    continue
                ...
                if not torch.isfinite(log_probs).all():
                    ...
                    self._log_batch_debug_info(i, batch, features, n_time_steps, adjusted_lens, phone_seq_lens, day_indicies)
                    self.logger.info(f"logits abs max before log_softmax: {logits.detach().abs().max().item():.6f}")
                    continue
            ...
            if self._check_nonfinite_gradients(i):
                self._log_batch_debug_info(i, batch, features, n_time_steps, adjusted_lens, phone_seq_lens, day_indicies)
                continue
            ...
            if i % self.args["batches_per_val_step"] == 0 or i == (self.args["num_training_batches"] - 1):
                ...
                new_best = False
                val_metrics_finite = np.isfinite(val_metrics["avg_PER"]) and np.isfinite(val_metrics["avg_loss"])
                ...
                if early_stopping and (val_steps_since_improvement >= early_stopping_val_steps):
                    self.logger.info(
                        f"Overall validation PER has not improved in {early_stopping_val_steps} validation steps. "
    ...
    def validation(self, loader, return_logits=False, return_data=False):
        ...
        metrics["decoded_phonemes"] = []
        metrics["true_phonemes"] = []
        ...
        for i, batch in enumerate(loader):
            ...
            batch_phone_edit_distance = 0
            batch_phone_length = 0
            ...
            for iterIdx in range(logits.shape[0]):
                ...
                pred_phone_seq = ctc_greedy_decode_phoneme_ids(pred_ids)

                true_len = int(phone_seq_lens[iterIdx].item())
                true_phone_ids = labels[iterIdx, :true_len].detach().cpu().numpy().tolist()
                true_phone_seq = [PHONE_VOCAB[int(x)] for x in true_phone_ids if int(x) != 0]

                batch_phone_edit_distance += calculate_error_rate(true_phone_seq, pred_phone_seq)
                batch_phone_length += max(len(true_phone_seq), 1)

                decoded_phonemes.append(phoneme_seq_to_string(pred_phone_seq))
                true_phonemes.append(phoneme_seq_to_string(true_phone_seq))

            day_metrics[day]["total_phone_edit_distance"] += batch_phone_edit_distance
            day_metrics[day]["total_phone_length"] += batch_phone_length

            total_phone_edit_distance += batch_phone_edit_distance
            total_phone_length += batch_phone_length
            ...
            metrics["decoded_phonemes"].append(decoded_phonemes)
            metrics["true_phonemes"].append(true_phonemes)
            metrics["phone_seq_lens"].append(phone_seq_lens.cpu().numpy())
            ...
            metrics["raw_text"].append([batch["raw_text"][idx] for idx in valid_indices])

        avg_PER = total_phone_edit_distance / max(total_phone_length, 1)
