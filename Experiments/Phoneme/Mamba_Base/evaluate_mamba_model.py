...
if os.path.exists(args_path):
    ...
    model_args = OmegaConf.load("mamba_args.yaml")
...
model = MambaDecoder(
    ...
    dropout=model_args['model']['rnn_dropout'],
)

# load model weights
checkpoint = torch.load(
    os.path.join(model_path, 'checkpoint/best_checkpoint'),
    map_location=f"cuda:{gpu_number}" if torch.cuda.is_available() else "cpu",
    weights_only=False)
...
output_file = os.path.join(model_path, f'baseline_mamba_{eval_type}_predicted_sentences_{time.strftime("%Y%m%d_%H%M%S")}.csv')
