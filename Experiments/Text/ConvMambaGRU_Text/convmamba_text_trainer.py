...
from ConvMambaGru_text_model import MambaTextDecoder
...
class BrainToTextDecoderTrainer:
    """
    Trainer for brain-to-text decoding with ConvMambaGRU + CTC
    """
    ...
    def __init__(self, args):
        ...
        self.model = MambaTextDecoder(
            ...
            d_state=self.args["model"].get("d_state", 16),
            d_conv=self.args["model"].get("d_conv", 4),
            expand=self.args["model"].get("expand", 2),
            ...
            gru_hidden_dim=self.args["model"].get("gru_hidden_dim", None),
            gru_layers=self.args["model"].get("gru_layers", 1),
            gru_dropout=self.args["model"].get("gru_dropout", 0.0),
        )

        self.logger.info("Initialized ConvMambaGRU text decoding model")
        ...
        if self.args["init_from_checkpoint"]:
            self.load_model_weights_only(self.args["init_checkpoint_path"])
    ...
    def load_model_weights_only(self, load_path):
        checkpoint = torch.load(load_path, weights_only=False)
        self.model.load_state_dict(checkpoint["model_state_dict"])
        self.best_val_WER = checkpoint["val_WER"] if "val_WER" in checkpoint else torch.inf
        self.best_val_CER = checkpoint["val_CER"] if "val_CER" in checkpoint else torch.inf
        self.best_val_loss = checkpoint["val_loss"] if "val_loss" in checkpoint else torch.inf
        self.model.to(self.device)
        self.logger.info("Loaded model weights only from checkpoint: " + load_path)
    ...
    def _log_batch_debug_info(self, i, batch, features, n_time_steps, adjusted_lens, char_seq_lens, day_indicies):
        try:
            ...
            self.logger.info(f"char_seq_lens min/max: {char_seq_lens.min().item()} / {char_seq_lens.max().item()}")
            self.logger.info(f"Any adjusted_lens <= 0? {bool((adjusted_lens <= 0).any().item())}")
            self.logger.info(f"Any char_seq_lens <= 0? {bool((char_seq_lens <= 0).any().item())}")
            self.logger.info(f"Any adjusted_lens < char_seq_lens? {bool((adjusted_lens < char_seq_lens).any().item())}")
            ...
            self.logger.info(f"labels min/max: {labels_minmax(batch['char_seq_ids'])}")
        except Exception as e:
    ...
    def train(self):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            if not torch.isfinite(features).all():
                ...
                self._log_batch_debug_info(i, batch, features, n_time_steps, n_time_steps.to(torch.int32), char_seq_lens, day_indicies)
                continue
            ...
            with torch.autocast(device_type="cuda", enabled=self.args["use_amp"], dtype=torch.bfloat16):
                ...
                if not torch.isfinite(features).all():
                    ...
                    self._log_batch_debug_info(i, batch, features, n_time_steps, adjusted_lens, char_seq_lens, day_indicies)
                    continue
    ...
    def validation(self, loader, return_logits=False, return_data=False):
        ...
        for i, batch in enumerate(loader):
            ...
            for iterIdx in range(logits.shape[0]):
                ...
                ref_text = self._extract_reference_text(batch, valid_indices[iterIdx])
